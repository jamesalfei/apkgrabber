...
public class AnimationUtil {
	...
	public static void startDefaultAnimation(Context context, ViewGroup v) {
	...
	public static void startSlideAnimation(Context context, ViewGroup v) {
		if (new UpdaterOptions(context).disableAnimations()) {
			// No animation
		} else if (Build.VERSION.SDK_INT >= 21) {
	...
	public static void startToolbarAnimation(Context context, ViewGroup v) {
		if (new UpdaterOptions(context).disableAnimations()) {
			return;
		}

		if (Build.VERSION.SDK_INT >= 21) {
			TransitionManager.beginDelayedTransition(v, new TransitionSet().addTransition(new Fade(Fade.IN)).addTransition(new Slide(Gravity.LEFT)).addTransition(new ChangeBounds()));

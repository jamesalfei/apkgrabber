...
public class VersionUtil {
	...
	static public String getStringVersionFromString(String full_string) {
	...
	static public List<Integer> getVersionFromString(String full_string) {
	...
	static public int compareVersion(List<Integer> l, List<Integer> r) {
	...
	static public boolean isExperimental(String s) {
		return s.toLowerCase().contains("beta") || s.toLowerCase().contains("alpha");
	}

	static public String getUserAgent(Context context) {
	...
	static public <T> List<List<T>> batchList(List<T> list, int batchSize) {
	...
	static public boolean skipArchitecture(List<String> arches) {
		if (arches.isEmpty()) {
			return false;
		}
	...
	static public boolean skipMinapi(String minApi) {
		try {
			...
			return Integer.valueOf(minApi) > Build.VERSION.SDK_INT;
		} catch (Exception e) {
			return false;
		}
	}

...
public class GooglePlayUtil {
	...
	static public GooglePlayAPI getApi(Context context) {
		...
		while (api == null && c < 10) {
			try {
				...
				com.github.yeriomin.playstoreapi.PlayStoreApiBuilder builder = new com.github.yeriomin.playstoreapi.PlayStoreApiBuilder().setHttpClient(new OkHttpClientAdapter()).setDeviceInfoProvider(getNativeProvider(context)).setLocale(Locale.getDefault()).setEmail(null).setPassword(null);
	...
	public static AndroidAppDeliveryData getAppDeliveryData(GooglePlayAPI api, String pname) throws IOException {
		DocV2 d = api.details(pname).getDocV2();
		return api.purchase(d.getDetails().getAppDetails().getPackageName(), d.getDetails().getAppDetails().getVersionCode(), d.getOffer(0).getOfferType()).getPurchaseStatusResponse().getAppDeliveryData();
	}

	public static Pair<String, String> getIdTokenPairFromEmailPassword(Context context, String email, String password) throws Exception {

		com.github.yeriomin.playstoreapi.PlayStoreApiBuilder builder = new com.github.yeriomin.playstoreapi.PlayStoreApiBuilder().setHttpClient(new OkHttpClientAdapter()).setDeviceInfoProvider(getNativeProvider(context)).setLocale(Locale.getDefault()).setEmail(email).setPassword(password);

...
public class InstalledAppUtil {
	...
	static public List<InstalledApp> sort(Context context, List<InstalledApp> items) {
		if (items == null) {
			return null;
		}
	...
	static public int getAppVersionCode(Context context, String pName) {
		try {
	...
	static public String getAppVersionName(Context context, String pName) {
		try {
			PackageManager pm = context.getPackageManager();
			PackageInfo i = pm.getPackageInfo(pName, 0);
	...
	@Background
	public void getInstalledAppsAsync(final Context context, final GenericCallback<List<InstalledApp>> callback) {

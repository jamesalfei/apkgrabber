...
public class AutomaticInstallerService extends IntentService {
	...
	private void doAppNotification(int current, int max, Update u) {
	...
	private void doFinalNotification(List<Update> success, List<Update> failure) {
		Context c = getApplicationContext();
		NotificationCompat.Builder b = new NotificationCompat.Builder(getApplicationContext());
	...
	private void installApps(Update[] updates) {
		try {
			...
			for (Update u : updates) {
				try {
					...
					AndroidAppDeliveryData data = GooglePlayUtil.getAppDeliveryData(GooglePlayUtil.getApi(getApplicationContext()), u.getPname());

					File f = DownloadUtil.downloadFile(getApplicationContext(), data.getDownloadUrl(), data.getDownloadAuthCookie(0).getName() + "=" + data.getDownloadAuthCookie(0).getValue());
			...
			Boolean switchPref = sharedPreferences.getBoolean(getApplicationContext().getString(R.string.preferences_general_self_update_key), true);
			if (switchPref) {

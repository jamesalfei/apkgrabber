...
public class MainActivity extends AppCompatActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (!(getSupportFragmentManager().findFragmentById(R.id.container) instanceof MainFragment)) {
			getSupportFragmentManager().beginTransaction().replace(R.id.container, mMainFragment).add(R.id.container, mSettingsFragment).add(R.id.container, mLogFragment).add(R.id.container, mAboutFragment).show(mMainFragment).hide(mSettingsFragment).hide(mLogFragment).hide(mAboutFragment).commit();
		}
	...
	private void colorFloatingActionButton() {
		if (ThemeUtil.getActivityThemeFromOptions(this) == R.style.AppThemeBloody) {
			mUpdateButton.setBackgroundTintList(ColorStateList.valueOf(ColorUtil.getColorFromContext(this, android.R.attr.textColorPrimary)));
			mUpdateButton.setImageDrawable(ColorUtil.tintDrawable(this, mUpdateButton.getDrawable(), R.attr.colorPrimary));
		} else {
			mUpdateButton.setImageDrawable(ColorUtil.tintDrawable(this, mUpdateButton.getDrawable(), android.R.attr.textColorPrimary));
		}
	}

	private void doTransition(int[] anim, Fragment show, Fragment[] hide) {
	...
	private void switchLog(boolean b) {
		if (b) {
	...
	private void switchAbout(boolean b) {
		if (b) {
	...
	private void changeToolbar(String title, boolean arrow) {
		try {
	...
	@Subscribe
	public void onPackageInstallerEvent(PackageInstallerEvent ev) {
		try {
		...
		if (mAppState.getDownloadIds().containsKey(requestCode)) {
			...
			if (mAppState.getDownloadInfo().containsKey(id)) {
				...
				if (i.getPackageName().equals(getPackageName())) {
					...
					mBus.post(new SnackBarEvent(version.equals(i.getVersionName()) ? getString(R.string.install_success) : getString(R.string.install_failure)));
				} else {

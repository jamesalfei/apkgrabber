...
public class DownloadReceiver extends BroadcastReceiver {
	...
	private boolean openDownloadedFile(final Context context, final Intent intent) {
		long id = -1;
		try {
	...
	private boolean openDownloadedFileDirect(final Context context, final Intent intent) {
		long id = -1;
		try {
	...
	private void install(Context context, String uriString, long id) throws Exception {
		...
		if (options.useRootInstall()) {
			...
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
				...
				install.setDataAndType(FileProvider.getUriForFile(context, Constants.FileProvider, new File(u.getPath())), Constants.ApkMime);
	...
	private void installWithRoot(String uriString) throws Exception {
		try {
